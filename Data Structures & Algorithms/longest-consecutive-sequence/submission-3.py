...
# Each element is visited once in the for loop.

# The while loop doesn’t make it O(n²) because every number is part of only one sequence expansion.
...
# The set stores up to n unique numbers.

# Only a few variables (max_len, sequence_len) use constant space.

# Overall → linear space for the hash set.
